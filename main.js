...
function leaf(args) {
  ...
  for (var i = 0; i < seg; i++) {
    ...
    if (i > 0) {
      ...
      polygon({
        ctx: ctx, pts: [l, L[-1], P[-1], disp],
        xof: xof, yof: yof, fil: true, str: true, col: hsv(h, s, v, a)
      })
      polygon({
        ctx: ctx, pts: [r, R[-1], P[-1], disp],
        xof: xof, yof: yof, fil: true, str: true, col: hsv(h, s, v, a)
      })
    }
    P.push(disp);
  ...
  if (vei[0] == 1) {
    ...
    for (var i = 1; i < P.length - vei[1]; i += vei[2]) {
      polygon({
        ctx: ctx, pts: [P[i], L[i + vei[1]]],
        xof: xof, yof: yof, fil: false, col: hsv(0, 0, 0, normRand(0.4, 0.9))
      })
      polygon({
        ctx: ctx, pts: [P[i], R[i + vei[1]]],
        xof: xof, yof: yof, fil: false, col: hsv(0, 0, 0, normRand(0.4, 0.9))
      })
    }
    stroke({ ctx: ctx, pts: P, xof: xof, yof: yof, col: rgba(0, 0, 0, 0.3) })
...
function herbal(args) {
  ...
  for (var i = 0; i < PAR.stemCount; i++) {
    ...
    if (PAR.sheathLength != 0) {
      stem({
        ctx: lay0, xof: x0 + P[-1].x, yof: y0 + P[-1].y,
        rot: hr,
...
function download() {
  ...
  let now = new Date().toLocaleString().replaceAll("/", "-").replaceAll(",", "").replaceAll(":", " ");
  ele.download = `${now}.png`;
  ele.href = document.getElementById("c").toDataURL(`image/png;base64`);
  ele.click();
  ele.remove();
}
